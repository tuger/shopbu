    ...
    <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
      ...
      <Card className="w-full sm:w-8/12 md:w-2/3">
        <Input
          ...
          error={t((errors as any)?.page_options?.userStoryTitle?.message)}
          variant="outline"
          className="mb-5"
        />
        <TextArea
          ...
          error={t(
            (errors as any)?.page_options?.userStoryDescription?.message,
          )}
          className="mb-5"
        />
        ...
        >
          {fields?.map(
            (
              ...
              let error =
                (errors as any)?.page_options?.userStories?.[index]?.title
                  ?.message ||
                (errors as any)?.page_options?.userStories?.[index]?.description
                  ?.message ||
                (errors as any)?.page_options?.userStories?.[index]?.link
                  ?.message;
              return (
                <div
                  ...
                  >
                    <div className="grid grid-cols-1 gap-5 sm:col-span-4">
                      <Input
                        ...
                        required
                        error={t(
                          (errors as any)?.page_options?.userStories?.[index]
                            ?.title?.message,
                        )}
                      />
                      <TextArea
          ...
          {(errors as any)?.page_options?.userStories?.message ||
          (errors as any)?.page_options?.userStories?.root?.message ? (
            <Alert
              message={t(
                (errors as any)?.page_options?.userStories?.message ||
                  (errors as any)?.page_options?.userStories?.root?.message,
