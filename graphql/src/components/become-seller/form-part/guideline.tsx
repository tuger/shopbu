    ...
    <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
      ...
      <Card className="w-full sm:w-8/12 md:w-2/3">
        <Input
          ...
          error={t((errors as any)?.page_options?.guidelineTitle?.message)}
          variant="outline"
          className="mb-5"
          required
        />
        <TextArea
          ...
          error={t(
            (errors as any)?.page_options?.guidelineDescription?.message,
          )}
          className="mb-5"
          required
        />
        ...
        >
          {fields?.map((item: any & { id: string }, index: number) => {
            let error =
              (errors as any)?.page_options?.guidelineItems?.[index]?.title
                ?.message ||
              (errors as any)?.page_options?.guidelineItems?.[index]?.link
                ?.message;
                ...
                >
                  <div className="space-y-5 sm:col-span-4">
                    <Input
                      ...
                      error={t(
                        (errors as any)?.page_options?.guidelineItems?.[index]
                          ?.title?.message,
                      )}
                      required
                    />
          ...
          {(errors as any)?.page_options?.guidelineItems?.message ||
          (errors as any)?.page_options?.guidelineItems?.root?.message ? (
            <Alert
              message={t(
                (errors as any)?.page_options?.guidelineItems?.message ||
                  (errors as any)?.page_options?.guidelineItems?.root?.message,
