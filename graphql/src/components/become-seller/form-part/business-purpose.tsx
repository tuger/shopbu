...
import { BusinessPurposeItem } from '@/types/custom-types';
    ...
    <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
      ...
      <Card className="w-full sm:w-8/12 md:w-2/3">
        <Input
          ...
          error={t((errors as any)?.page_options?.purposeTitle?.message)}
          variant="outline"
          className="mb-5"
          required
        />
        ...
        <TextArea
          ...
          error={t((errors as any)?.page_options?.purposeDescription?.message)}
          className="mb-5"
          required
        />
        ...
        >
          {fields?.map((item: any & { id: string }, index: number) => {
            let error =
              (errors as any)?.page_options?.purposeItems?.[index]?.icon?.value
                ?.message ||
              (errors as any)?.page_options?.purposeItems?.[index]?.title
                ?.message ||
              (errors as any)?.page_options?.purposeItems?.[index]?.description
                ?.message;
            return (
              <div
                  ...
                  <div className="space-y-5 sm:col-span-4">
                    ...
                    <div>
                      <SelectInput
                        ...
                        error={t(
                          (errors as any)?.page_options?.purposeItems?.[index]
                            ?.icon?.value?.message,
                        )}
                        required
                      />
          ...
          {(errors as any)?.page_options?.purposeItems?.message ||
          (errors as any)?.page_options?.purposeItems?.root?.message ? (
            <Alert
              message={t(
                (errors as any)?.page_options?.purposeItems?.message ||
                  (errors as any)?.page_options?.purposeItems?.root?.message,
