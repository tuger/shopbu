    ...
    <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
      ...
      <Card className="w-full sm:w-8/12 md:w-2/3">
        <Input
          ...
          error={t((errors as any)?.page_options?.sellingStepsTitle?.message)}
          variant="outline"
          className="mb-5"
          required
        />
        <TextArea
          ...
          error={t(
            (errors as any)?.page_options?.sellingStepsDescription?.message,
          )}
          className="mb-5"
          required
        />
        ...
        >
          {fields?.map((item: any & { id: string }, index: number) => {
            let error =
              (errors as any)?.page_options?.sellingStepsItem?.[index]?.image
                ?.message ||
              (errors as any)?.page_options?.sellingStepsItem?.[index]?.title
                ?.message ||
              (errors as any)?.page_options?.sellingStepsItem?.[index]
                ?.description?.message;
            return (
              <div
          ...
          {(errors as any)?.page_options?.sellingStepsItem?.message ||
          (errors as any)?.page_options?.sellingStepsItem?.root?.message ? (
            <Alert
              message={t(
                (errors as any)?.page_options?.sellingStepsItem?.message ||
                  (errors as any)?.page_options?.sellingStepsItem?.root
                    ?.message,
              )}
              variant="error"
              className="my-5"
